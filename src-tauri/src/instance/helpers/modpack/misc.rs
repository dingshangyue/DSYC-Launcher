...
pub trait ModpackManifest {
  fn from_archive(file: &File) -> DSYCLauncherResult<Self>
  where
    Self: Sized;
  fn get_client_version(&self) -> DSYCLauncherResult<String>;
  fn get_mod_loader_type_version(&self) -> DSYCLauncherResult<(ModLoaderType, String)>;
  async fn get_meta_info(&self, app: &AppHandle) -> DSYCLauncherResult<ModpackMetaInfo>;
  async fn get_download_params(
    &self,
    app: &AppHandle,
    instance_path: &Path,
  ) -> DSYCLauncherResult<Vec<PTaskParam>>;
  fn get_overrides_path(&self) -> String;
}

type ManifestBox = Box<dyn ModpackManifest + Send + Sync>;
type Parser = Box<dyn Fn(&File) -> DSYCLauncherResult<ManifestBox> + Send + Sync>;
...
impl ModLoader {
  pub async fn with_branch(&self, app: &AppHandle, mc_version: String) -> DSYCLauncherResult<Self> {
    let version_list =
...
impl ModpackMetaInfo {
  pub async fn from_archive(app: &AppHandle, file: &File) -> DSYCLauncherResult<Self> {
...
pub fn extract_overrides(file: &File, instance_path: &Path) -> DSYCLauncherResult<()> {
