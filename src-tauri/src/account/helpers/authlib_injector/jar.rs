...
pub fn get_jar_path(app: &AppHandle) -> DSYCLauncherResult<PathBuf> {
  Ok(
    app
      .path()
...
) -> DSYCLauncherResult<AuthlibInjectorMeta> {
...
fn get_local_version(app: &AppHandle) -> DSYCLauncherResult<String> {
...
async fn download(app: &AppHandle, url: Url) -> DSYCLauncherResult<()> {
  let client = app.state::<reqwest::Client>();
  let jar_path = get_jar_path(app)?;
...
pub async fn check_authlib_jar(app: &AppHandle) -> DSYCLauncherResult<()> {
  let latest_meta = {
