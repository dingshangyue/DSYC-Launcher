...
pub fn setup_with_app(app: AppHandle) -> DSYCLauncherResult<()> {
  ...
  app
    ...
    .map_err(|e| DSYCLauncherError(format!("Failed to setup log plugin: {}", e)))?;

  Ok(())
}

pub async fn purge_old_launcher_logs(app: AppHandle, days: u64) -> DSYCLauncherResult<()> {
  let folder = get_launcher_logs_folder(&app);
  if !folder.exists() {
    return Ok(());
  }
