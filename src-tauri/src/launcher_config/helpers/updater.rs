...
const SOURCES: [SourceTuple; 2] = [
  ...
  (
    "https://api.github.com/repos/UNIkeEN/DSYCLauncher/releases/latest",
    "tag_name",
    |ver, fname| {
      format!(
        "https://github.com/UNIkeEN/DSYCLauncher/releases/download/v{}/{}",
        ver, fname
      )
    },
  ),
...
fn build_resource_filename(ver: &str, os: &str, arch: &str, is_portable: bool) -> String {
  ...
  format!("DSYCLauncher_{}_{}_{}{}", ver, os, arch, suffix)
}
...
pub async fn fetch_latest_version(
  app: &AppHandle,
) -> DSYCLauncherResult<Option<(String, String, String, String)>> {
...
pub async fn install_update_windows(
  ...
  if is_portable {
    ...
    let cur_dir = cur_exe
      .parent()
      .ok_or_else(|| DSYCLauncherError("No parent dir for exe".to_string()))?;
    let old_name = cur_exe
      .file_name()
      .and_then(|s| s.to_str())
      .ok_or_else(|| DSYCLauncherError("Invalid exe name".to_string()))?
      .to_string();
...
pub async fn install_update_macos(
  ...
  let app_bundle = cur_exe
    ...
    .ok_or_else(|| DSYCLauncherError("Not inside .app bundle".to_string()))?
    .to_path_buf();
  let app_dir = app_bundle
    .parent()
    .ok_or_else(|| DSYCLauncherError("No parent dir for .app".to_string()))?
    .to_path_buf();
  let old_name = app_bundle
    .file_name()
    .and_then(|s| s.to_str())
    .ok_or_else(|| DSYCLauncherError("Invalid .app name".to_string()))?
    .to_string();
