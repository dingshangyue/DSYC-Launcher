<img src="figs/banner.png" alt="DSYCLauncher" />
...
開始使用 DSYCLauncher，只需前往 [官網](https://mc.sjtu.cn/sjmcl/en) 下載最新版即可。

您也可以在 [GitHub Releases](https://github.com/UNIkeEN/DSYCLauncher/releases) 獲取所有版本，包括週期性構建。

DSYCLauncher 目前支援以下平臺：
...
若您需要在 Windows 7 上執行 DSYCLauncher，請先 [下載 Microsoft Edge WebView2 運行時](https://developer.microsoft.com/zh-tw/microsoft-edge/webview2#download) 並安裝，建議選擇「常青引導程式」。
...
DSYCLauncher 已上傳至 Arch Linux 使用者套件庫（AUR）。您可以使用常見的 [AUR 助手](https://wiki.archlinux.org/title/AUR_helpers) 安裝：
...
* 在開始前，請先閱讀我們的 [貢獻指南](https://github.com/UNIkeEN/DSYCLauncher/blob/main/CONTRIBUTING.md)（內含開發流程詳細說明）。
* API 參考與部分開發者筆記見 [開發者文件](https://mc.sjtu.cn/sjmcl/zh/dev)。
* 歡迎透過 [Pull Request](https://github.com/UNIkeEN/DSYCLauncher/pulls) 或 [GitHub Issues](https://github.com/UNIkeEN/DSYCLauncher/issues) 分享您的想法。
...
版權所有 © 2024-2026 DSYCLauncher 團隊。
...
1. 必須更換軟體名稱，禁止使用 DSYCLauncher 或 DSYC Launcher；
2. 在您的倉庫 README、分發網站或相關文件、軟體的關於頁面中，須明確標註您的程式基於 DSYCLauncher，並註明原倉庫連結。
...
DSYCLauncher 使用者 QQ 交流群：860851380

您還可以傳送郵件至 [launcher@sjmc.club](mailto:launcher@sjmc.club) 來聯絡我們。

## 社群合作伙伴

衷心感謝以下組織對 DSYCLauncher 專案開發與社群的支援。
