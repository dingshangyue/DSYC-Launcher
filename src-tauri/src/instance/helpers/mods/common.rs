...
pub async fn get_mod_info_from_jar(path: &PathBuf) -> DSYCLauncherResult<LocalModInfo> {
...
pub async fn get_mod_info_from_dir(path: &Path) -> DSYCLauncherResult<LocalModInfo> {
...
pub async fn add_local_mod_translations(
  ...
  let modrinth_result = {
    ...
    tokio::spawn(async move {
      ...
      Ok::<_, DSYCLauncherError>(resource_info)
    })
  };
