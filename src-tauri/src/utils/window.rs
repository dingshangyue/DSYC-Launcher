...
pub async fn create_webview_window(
  ...
  config_label: &str,
  url: Option<Url>,
) -> DSYCLauncherResult<WebviewWindow> {
  let window_config = app
    ...
    .ok_or_else(|| DSYCLauncherError(format!("Config label '{}' not found", config_label)))?;
...
pub async fn create_webview_window_with_config(
  ...
  let builder = WebviewWindowBuilder::from_config(app, config).map_err(DSYCLauncherError::from)?;
  let window = builder.build().map_err(DSYCLauncherError::from)?;
