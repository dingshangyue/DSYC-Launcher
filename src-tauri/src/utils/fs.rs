use crate::error::{DSYCLauncherError, DSYCLauncherResult};
...
/// ```
pub fn get_subdirectories<P: AsRef<Path>>(path: P) -> DSYCLauncherResult<Vec<PathBuf>> {
  fs::read_dir(path)?
    .filter_map(|entry| match entry {
      ...
      Err(e) => Some(Err(DSYCLauncherError(format!("Entry Error: {}", e)))),
    })
    .collect()
}
...
pub fn get_files_with_regex<P: AsRef<Path>>(
  ...
  let dir_entries = fs::read_dir(&path).map_err(|e| {
    ...
    DSYCLauncherError(error_message)
  })?;

  let mut matching_files = Vec::new();

  for entry in dir_entries {
    let entry = entry.map_err(|e| DSYCLauncherError(format!("Read Entry Error: {}", e)))?;
    let path = entry.path();
...
pub fn create_url_shortcut(
  ...
  let desktop = app
    .path()
    .desktop_dir()
    .map_err(|e| DSYCLauncherError(format!("Failed to get desktop path: {}", e)))?;
  ...
  // process icon, use DSYCLauncher icon as default (macOS webloc does not support icon)
  #[cfg(target_os = "macos")]
  {
  ...
  let final_icon_path: PathBuf = match icon_path {
    ...
    None => {
      ...
      let resource_icon = get_app_resource_filepath(app, &format!("assets/icons/{}", icon_name))
        .map_err(|e| DSYCLauncherError(format!("Failed to resolve resource icon: {}", e)))?;

      let appdata_icon = app
        .path()
        .resolve(icon_name, BaseDirectory::AppData)
        .map_err(|e| DSYCLauncherError(format!("Failed to resolve appdata icon path: {}", e)))?;

      fs::copy(&resource_icon, &appdata_icon)
        .map_err(|e| DSYCLauncherError(format!("Failed to copy default icon: {}", e)))?;
  ...
  #[cfg(target_os = "windows")]
  {
    ...
    fs::write(&path, content).map_err(|e| DSYCLauncherError(e.to_string()))?;
  }
  ...
  //   let file = fs::File::create(&path).map_err(|e| DSYCLauncherError(e.to_string()))?;
  //   plist_value.to_writer_xml(file).map_err(|e| DSYCLauncherError(e.to_string()))?;
  // }
  ...
  #[cfg(target_os = "macos")]
  {
    ...
    let mut file = fs::File::create(&path).map_err(|e| DSYCLauncherError(e.to_string()))?;
    file
      .write_all(content.as_bytes())
      .map_err(|e| DSYCLauncherError(e.to_string()))?;
    ...
    fs::set_permissions(&path, perms).map_err(|e| DSYCLauncherError(e.to_string()))?;
  }
...
pub fn validate_sha1(dest_path: PathBuf, truth: String) -> DSYCLauncherResult<()> {
  ...
  std::io::copy(&mut f, &mut hasher)
    .map_err(|e| DSYCLauncherError(format!("Failed to copy data for SHA1 validation: {}", e)))?;

  let sha1 = hex::encode(hasher.finalize());
  if sha1 != truth {
    Err(DSYCLauncherError(format!(
...
///
/// # Returns
/// - `DSYCLauncherResult<String>`: The SHA256 hash as a hexadecimal string, or an error
pub fn calculate_sha256(path: &Path) -> DSYCLauncherResult<String> {
  match std::fs::File::open(path) {
    Ok(mut file) => {
      ...
      loop {
        match file.read(&mut buffer) {
          ...
          Err(e) => {
            return Err(DSYCLauncherError(format!(
    ...
    Err(e) => Err(DSYCLauncherError(format!(
  ...
  for path in paths {
    if path.is_file() {
      ...
      let mut file = std::fs::File::open(&path)
        .map_err(|e| DSYCLauncherError(format!("Failed to open file {}: {}", path.display(), e)))?;
      std::io::copy(&mut file, &mut zip)
        .map_err(|e| DSYCLauncherError(format!("Failed to copy data to zip: {}", e)))?;
    }
  }

  zip
    .finish()
    .map_err(|e| DSYCLauncherError(format!("Failed to finalize zip file: {}", e)))?;
