...
) -> DSYCLauncherResult<Vec<GameClientResourceInfo>> {
  let priority_list = {
...
pub async fn fetch_game_version_specific(
  ...
  state: State<'_, Mutex<LauncherConfig>>,
  game_version: String,
) -> DSYCLauncherResult<GameClientResourceInfo> {
...
) -> DSYCLauncherResult<Vec<ModLoaderResourceInfo>> {
  let priority_list = {
...
pub async fn fetch_optifine_version_list(
  app: AppHandle,
  game_version: String,
) -> DSYCLauncherResult<Vec<OptiFineResourceInfo>> {
  let priority_list = {
    let launcher_config_state = app.state::<Mutex<LauncherConfig>>();
    let launcher_config = launcher_config_state.lock()?;
    get_source_priority_list(&launcher_config)
...
) -> DSYCLauncherResult<OtherResourceSearchRes> {
...
) -> DSYCLauncherResult<Vec<OtherResourceVersionPack>> {
  match download_source {
    OtherResourceSource::CurseForge => {
...
pub async fn fetch_remote_resource_by_local(
  ...
  download_source: OtherResourceSource,
  file_path: String,
) -> DSYCLauncherResult<OtherResourceFileInfo> {
  match download_source {
    OtherResourceSource::CurseForge => {
...
pub async fn fetch_remote_resource_by_id(
  ...
  download_source: OtherResourceSource,
  resource_id: String,
) -> DSYCLauncherResult<OtherResourceInfo> {
  match download_source {
    OtherResourceSource::CurseForge => {
