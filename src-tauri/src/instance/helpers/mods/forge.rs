...
pub fn get_mod_metadata_from_jar<R: Read + Seek>(
...
) -> DSYCLauncherResult<ForgeModMetadata> {
  ...
  if meta.mods.is_empty() {
    return Err(DSYCLauncherError("forge mod len(mods) == 0".to_string()));
  }
  // seek logo
...
pub async fn get_mod_metadata_from_dir(dir_path: &Path) -> DSYCLauncherResult<ForgeModMetadata> {
  let mut meta_result = None;
