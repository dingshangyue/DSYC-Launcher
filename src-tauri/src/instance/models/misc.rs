...
pub enum ModLoaderStatus {
  ...
  DownloadFailed, /* mod loader's library download process failed (including processor installation failed)
                  Only when DSYCLauncher restart, it will try to re-download library while making no changes to client info JSON (is_retry = true),
...
impl InstanceSummary {
  pub async fn from_instance(
    ...
    InstanceSummary {
      ...
      // ref: https://github.com/UNIkeEN/DSYCLauncher/pull/799
