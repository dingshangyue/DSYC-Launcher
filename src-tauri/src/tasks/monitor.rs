...
use crate::tasks::{DSYCLauncherFuture, *};
...
pub struct TaskMonitor {
  ...
  tx: FlumeSender<DSYCLauncherFuture>,
  rx: FlumeReceiver<DSYCLauncherFuture>,
  group_map: Arc<RwLock<HashMap<String, GroupMonitor>>>,
  stopped_futures: Arc<Mutex<Vec<DSYCLauncherFuture>>>,
...
impl TaskMonitor {
  ...
  pub async fn enqueue_task<T>(
    ...
    T: Future<Output = DSYCLauncherResult<()>> + Send + 'static,
  {
  ...
  pub async fn enqueue_task_group(&self, task_group: String, futures: Vec<DSYCLauncherFutureDesc>) {
