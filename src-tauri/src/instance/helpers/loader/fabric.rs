...
pub async fn install_fabric_loader(
  ...
  let loader_path = meta["loader"]["maven"]
    .as_str()
    .ok_or(DSYCLauncherError("meta missing loader maven".to_string()))?;
  ...
  let main_class = meta["launcherMeta"]["mainClass"]["client"]
    .as_str()
    .ok_or(DSYCLauncherError("missing mainClass.client".to_string()))?;
  ...
  let mut push_task = |coord: &str, url_root: &str| -> DSYCLauncherResult<()> {
...
pub async fn remove_fabric_api_mods<P: AsRef<Path>>(mods_dir: P) -> DSYCLauncherResult<()> {
  let mods_dir = mods_dir.as_ref();
  if !mods_dir.exists() {
    return Ok(());
  }
  let re = Regex::new(r"(?i)^(fabric-api|quilted-fabric-api)-.*\.jar$")
    .map_err(|e| DSYCLauncherError(format!("Invalid regex: {}", e)))?;
