...
pub async fn device_authorization(app: &AppHandle) -> DSYCLauncherResult<DeviceAuthResponseInfo> {
...
async fn fetch_xbl_token(app: &AppHandle, microsoft_token: String) -> DSYCLauncherResult<String> {
  let client = app.state::<reqwest::Client>();
...
) -> DSYCLauncherResult<MinecraftProfile> {
  let client = app.state::<reqwest::Client>();
...
async fn parse_profile(app: &AppHandle, tokens: &OAuthTokens) -> DSYCLauncherResult<PlayerInfo> {
...
pub async fn refresh(app: &AppHandle, player: &PlayerInfo) -> DSYCLauncherResult<PlayerInfo> {
  let client = app.state::<reqwest::Client>();
...
pub async fn validate(app: &AppHandle, player: &PlayerInfo) -> DSYCLauncherResult<bool> {
  let client = app.state::<reqwest::Client>();
  let response = client
    .get(PROFILE_ENDPOINT)
    .header(
