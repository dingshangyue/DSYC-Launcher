  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      <View style={styles.header}>
        <Text style={styles.title}>实例管理</Text>
      </View>
      
      <Card style={styles.card}>
        <Card.Content>
          <View style={styles.cardHeader}>
            <Text style={styles.cardTitle}>我的实例</Text>
          ...
          <List.Item
            ...
            description="Forge 49.0.1"
            left={() => <Avatar.Text size={40} label="MC" />}
            right={() => <Ionicons name="chevron-forward" size={20} color="#999" />}
          />
          <List.Item
            title="Minecraft 1.19.4"
            description="Fabric 0.14.22"
            left={() => <Avatar.Text size={40} label="MC" />}
            right={() => <Ionicons name="chevron-forward" size={20} color="#999" />}
          />
          <List.Item
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Text style={styles.cardTitle}>实例操作</Text>
          <View style={styles.actionGrid}>
            <TouchableOpacity style={styles.actionButton}>
              <Ionicons name="copy-outline" size={32} color="#007AFF" />
              <Text style={styles.actionText}>复制实例</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.actionButton}>
              <Ionicons name="download-outline" size={32} color="#007AFF" />
              <Text style={styles.actionText}>导入实例</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.actionButton}>
              <Ionicons name="cloud-upload-outline" size={32} color="#007AFF" />
              <Text style={styles.actionText}>导出实例</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.actionButton}>
              <Ionicons name="trash-outline" size={32} color="#007AFF" />
              <Text style={styles.actionText}>删除实例</Text>
            </TouchableOpacity>
          </View>
        </Card.Content>
      </Card>
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Text style={styles.cardTitle}>实例设置</Text>
          <List.Item
            title="内存分配"
            description="设置实例内存使用"
            left={() => <List.Icon icon="memory" />}
            right={() => <Ionicons name="chevron-forward" size={20} color="#999" />}
          />
          <List.Item
            title="Java版本"
            description="选择实例使用的Java"
            left={() => <List.Icon icon="coffee" />}
            right={() => <Ionicons name="chevron-forward" size={20} color="#999" />}
          />
          <List.Item
            title="启动参数"
            description="自定义JVM启动参数"
