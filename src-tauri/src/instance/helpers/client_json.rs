...
impl InstructionRule {
  pub fn is_allowed(&self, target_feature: &FeaturesInfo) -> DSYCLauncherResult<(bool, bool)> {
...
pub trait IsAllowed {
  fn is_allowed(&self, feature: &FeaturesInfo) -> DSYCLauncherResult<bool>;
}
...
impl LaunchArgumentTemplate {
  pub fn to_jvm_arguments(&self, feature: &FeaturesInfo) -> DSYCLauncherResult<Vec<String>> {
  ...
  pub fn to_game_arguments(&self, feature: &FeaturesInfo) -> DSYCLauncherResult<Vec<String>> {
    let mut arguments = Vec::new();
