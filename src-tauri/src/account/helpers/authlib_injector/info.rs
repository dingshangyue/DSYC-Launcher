...
pub async fn fetch_auth_server_info(
  app: &AppHandle,
  auth_url: String,
) -> DSYCLauncherResult<AuthServerInfo> {
...
pub async fn fetch_auth_url(app: &AppHandle, root: Url) -> DSYCLauncherResult<String> {
  let client = app.state::<reqwest::Client>();
  let response = client
    .get(root.clone())
    .send()
...
pub async fn refresh_and_update_auth_servers(app: &AppHandle) -> DSYCLauncherResult<()> {
